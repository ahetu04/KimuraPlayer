...
HRESULT ExtractDDSMipmaps(
    const Image* images,
    size_t nimages,
    const TexMetadata& metadata,
    std::vector<Kimura::Mipmap>& OutMipmaps) noexcept;




int texconv(_In_ int argc, wchar_t* argv[], std::vector<Kimura::Mipmap>& OutMipmaps)
{
        ...
        // --- Save result -------------------------------------------------------------
        {
            ...
            switch (FileType)
            {
                case CODEC_DDS:
                {
                    ...
                    ExtractDDSMipmaps(img, nimg, info, OutMipmaps);
    ...
    std::vector<Kimura::Mipmap>& OutMipmaps) noexcept
{
    ...
    case DDS_DIMENSION_TEXTURE2D:
    {
        ...
        OutMipmaps.resize(metadata.mipLevels);
        ...
        for (size_t item = 0; item < metadata.arraySize; ++item)
        {
            for (size_t level = 0; level < metadata.mipLevels; ++level, ++index)
            {
                ...
                {
                    OutMipmaps[level].width = (int)images[index].width;
                    OutMipmaps[level].height = (int)images[index].height;
                    OutMipmaps[level].rowPitch = (int)images[index].rowPitch;
                    OutMipmaps[level].slicePitch = (int)images[index].slicePitch;

                    OutMipmaps[level].buffer.resize(ddsSlicePitch);
                    memcpy(OutMipmaps[level].buffer.data(), images[index].pixels, ddsSlicePitch);
...
//                     }
                ...
                else
                {
                    ...
                    // allocate a buffer big enough for this mipmap
                    OutMipmaps[level].buffer.resize(lines * ddsRowPitch);

                    const uint8_t* __restrict sPtr = images[index].pixels;
                    unsigned char* pDest = OutMipmaps[level].buffer.data();

                    for (size_t j = 0; j < lines; ++j)
                    {
                        // add each scanline to the buffer
                        memcpy(pDest, sPtr, ddsRowPitch);
                        pDest += j * ddsRowPitch;
